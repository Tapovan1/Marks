...
export default function EnhancedAdminPanel() {
  ...
  const [selectedStandardForUpload, setSelectedStandardForUpload] =
    useState("");
  const [selectedClassForUpload, setSelectedClassForUpload] = useState("");
  const [selectedSubClassForUpload, setSelectedSubClassForUpload] =
    useState("");
  ...
  const fetchStudentsForClass = async (standard, className) => {
    ...
    try {
      const response = await fetch(
        `/api/students?standard=${standard}&class=${className}`
      );
    ...
    try {
      ...
      if (studentsWithoutClass.length > 0) {
        if (!selectedClassForUpload) {
          alert(
            "Some students don't have class specified in Excel. Please select a default class."
          );
          setIsLoading(false);
          return;
        }
      ...
      if (result.success) {
        setUploadResults({
          ...
          summary: `Uploaded ${result.count} students across ${
            Object.keys(studentsByClass).length +
            (studentsWithoutClass.length > 0 ? 1 : 0)
          } classes`,
        });
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <Select onValueChange={setSelectedClassForUpload}>
                      ...
                      <SelectContent>
                        {selectedStandardForUpload &&
                          standards[selectedStandardForUpload]?.classes.map(
                            (className) => (
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                      Only needed if your Excel doesn't have a Class column
                    </p>
                  </div>
                  {selectedStandardForUpload &&
                    (selectedStandardForUpload === "11" ||
                      selectedStandardForUpload === "12") && (
                      <div>
                        ...
                        <p className="text-sm text-gray-500 mt-1">
                          Only needed if your Excel doesn't have a SubClass
                          column (Column D)
                        </p>
                      </div>
                    )}
                  ...
                  <Button
                    ...
                    disabled={
                      !selectedFile || !selectedStandardForUpload || isLoading
                    }
                    className="w-full"
                  >
        ...
        <div className="space-y-2">
          ...
          {STANDARDS.slice(6).length > 0 && (
            <TabsList className="grid grid-cols-6 h-auto p-1 bg-muted rounded-md">
              ...
              {Array.from({ length: 6 - STANDARDS.slice(6).length }).map(
                (_, index) => (
        ...
        {STANDARDS.map((standard) => (
          <TabsContent key={standard} value={standard} className="mt-6">
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                ...
                >
                  ...
                  <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                      <DialogTitle>
                        Add New Student to Standard {standard}
                      </DialogTitle>
                    </DialogHeader>
              ...
              <Tabs
                defaultValue={standards[standard]?.classes[0]}
                className="w-full"
              >
                <TabsList>
                  {standards[standard]?.classes.map((className) => (
                      ...
                      <CardHeader>
                        <CardTitle>
                          ...
                          <span className="ml-2 text-sm font-normal text-gray-500">
                            ({getStudentsForClass(standard, className).length}{" "}
                            students)
                          </span>
                        </CardTitle>
                      </CardHeader>
                      <CardContent>
                        {isClassLoading(standard, className) ? (
                          ...
                          <Table>
                            ...
                            <TableBody>
                              {getStudentsForClass(standard, className).map(
                                (student) => (
